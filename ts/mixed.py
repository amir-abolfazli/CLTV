...
from ts.d4rl import get_d4rl_dataset, _normalize_obs
...
def get_mixed_d4rl_mujoco_datasets(agent, quality1, quality2, N, ratio, keep_traj=True, normalize_obs=False,
...
def get_mixed_d4rl_mujoco_datasets_from(agent, quality1, quality2, N, ratio, dataset1_env, dataset1, keep_traj=True,
                                        normalize_obs=False,
                                        normalize_reward=False):
    dataset_name1 = f"{agent}-{quality1}-v2"
    dataset_name2 = f"{agent}-{quality2}-v2"
    env, dataset1 = dataset1_env, dataset1
    _, dataset2 = get_d4rl_dataset(dataset_name2, normalize_obs=normalize_obs, normalize_reward=normalize_reward)
    ...
    if num1 and num2:
        ...
        for key in dataset1:
            dataset[key] = np.concatenate([dataset1[key][index1], dataset2[key][index2]], axis=0)
            dataset_source[key] = dataset1[key][index1]
            dataset_target[key] = dataset2[key][index2]
    else:
        if num1 == 0:
            dataset = dataset2
        else:
    ...
    if normalize_obs:
        dataset, info = _normalize_obs(dataset)
        dataset_source, info_source = _normalize_obs(dataset_source)
        dataset_target, info_target = _normalize_obs(dataset_target)
        from gym.wrappers.transform_observation import TransformObservation
        env = TransformObservation(env, lambda obs: (obs - info["obs_mean"]) / info["obs_std"])
    if normalize_reward == "std":
        dataset = std_reward_normalizer(dataset)
    ...
    return env, dataset, dataset_source, dataset_target
