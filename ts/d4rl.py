...
from ts.terminal import get_termination_fn
...
def get_d4rl_dataset(task, normalize_reward=False, normalize_obs=False, terminate_on_end: bool = False,
                     discard_last: bool = True, return_termination_fn=False, **kwargs):
    env = gym.make(task)
...
@torch.no_grad()
def gen_d4rl_dataset(task, policy, num_data, policy_is_online=False, random=False, normalize_obs: bool = False, seed=0,
                     **d4rl_kwargs):
